...
def inject_safe(filepath, refresh_action):
    ...
    # Match the main return of the default export
    # We find the LAST "return (" or similar by doing a regex search for return \s*\(
    matches = list(re.finditer(r'  return\s*\(', content))
    if not matches:
        print(f"No return found in {filepath}")
        return
    
    # The last match is usually the main render return.
    ret_idx = matches[-1].start()
    
    content = content[:ret_idx] + hook + content[ret_idx:]
    
    # Add to FlatList/ScrollView
    # We want the FIRST ScrollView or FlatList AFTER the return statement
...
inject_safe('app/admin/payments.tsx', "await loadPayments('reset');")
inject_safe('app/admin/analytics.tsx', "await fetchStats();")
inject_safe('app/admin/moderation.tsx', "await loadPendingCount(); await loadFeedback('reset');")
inject_safe('app/admin/manage-academics.tsx', "// Static dashboard, no explicit refetch needed; handled by sub-screens.")
inject_safe('app/admin/security.tsx', "await fetchSecurityLogs('reset');")
