    ...
    const load = async () => {
      try {
        ...
        if (!paymentsSnap.empty) {
          const latest = paymentsSnap.docs[0].data() as { state?: string; status?: string; type?: string; amount?: number };
          const paymentState = latest.state ?? latest.status ?? 'pending';
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: COLORS.surface, borderRadius: RADIUS.xxl, padding: SPACING.md, ...SHADOWS.card, gap: SPACING.sm },
  cardTitle: { ...TYPOGRAPHY.heading, fontSize: 18, color: COLORS.text },
  paymentCategory: { borderWidth: 1, borderColor: COLORS.border, borderRadius: RADIUS.xxl, padding: SPACING.sm, gap: SPACING.xs, backgroundColor: COLORS.surfaceAlt },
  ...
  choiceChip: { borderWidth: 1, borderColor: COLORS.border, borderRadius: RADIUS.full, paddingHorizontal: SPACING.md, paddingVertical: 8 },
